...
if ($command == "getGroups") {
	...
	$v = [Mapbender::session()->get("mb_lang"), $userId];
	$t = ["s", "i"];
	$res = db_prep_query($sql, $v, $t);
			
	while($row = db_fetch_array($res)){
		$resultArray = ["description" => $row["mb_group_description"], "name" => $row["mb_group_name"], "id" => $row["mb_group_id"]];
	...
	$v = [Mapbender::session()->get("mb_lang")];
	...
	while ($row = db_fetch_array($res)) {
		$resultArray = ["description" => $row["gui_description"], "name" => $row["gui_name"], "id" => $row["gui_id"]];
	...
	while ($row = db_fetch_array($res)) {
		...
		$resultArray = ["id" => $row["wms_id"], "title" => $row["wms_title"], "abstract" => $row["wms_abstract"], "getCapabilitiesUrl" => $wmsUrl, "version" => $row["wms_version"]];
	...
	$v = [Mapbender::session()->get("mb_lang"), $groupId];
	$t = ["s", "i"];
	$res = db_prep_query($sql, $v, $t);
	...
	$v = [Mapbender::session()->get("mb_lang"), $guiId];
	$t = ["s", "s"];
	$res = db_prep_query($sql, $v, $t);
