...
match ($command) {
    "getHtml" => getHtmlForm(),
    "searchGemeinden" => startSearch($command),
    "searchStrassen" => startSearch($command),
    "searchHauskoordinaten" => startSearch($command),
    "searchPlz" => startSearch($command),
    default => getHtmlForm(),
};
...
function startSearch($command) {
	...
	if (!preg_match($pattern, (string) $searchString)) {
		sendErrorMessage("Invalid searchString " . htmlentities((string) $searchString, ENT_QUOTES, CHARSET));
	}

	$searchFeaturetype = $_POST["searchFeaturetype"];
	$pattern = "/[a-z0-9]/i";
	if (!preg_match($pattern, (string) $searchFeaturetype)) {
		sendErrorMessage("Invalid searchFeaturetype " . htmlentities((string) $searchFeaturetype, ENT_QUOTES, CHARSET));
	}

	$admin = new administration();

	$searchConditions = explode("|", (string) $searchString);
	$propertyConditions = "";
