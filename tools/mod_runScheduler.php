...
while ($row = db_fetch_array($res)) {
	$resultObj = ["scheduler_id" 	=> $row['scheduler_id'], "wms_id"  =>  $row['wms_id'], "wms_title"  =>  $row['wms_title'], "last_change"  =>  $row['last_change'], "last_status"  =>  $row['last_status'], "wms_upload_url"  =>  $row['wms_upload_url'], "fkey_upload_id"  =>  $row['fkey_upload_id'], "wms_owner"  =>  $row['wms_owner'], "scheduler_interval"  =>  $row['scheduler_interval'], "scheduler_mail"  =>  $row['scheduler_mail'], "scheduler_publish"  =>  $row['scheduler_publish'], "scheduler_searchable"  =>  $row['scheduler_searchable'], "scheduler_overwrite"  =>  $row['scheduler_overwrite'], "scheduler_overwrite_categories"  =>  $row['scheduler_overwrite_categories'], "scheduler_status"  =>  $row['scheduler_status'], "wms_auth_type"  =>  $row['wms_auth_type'], "wms_username"  =>  $row['wms_username'], "wms_password"  =>  $row['wms_password']];
    ...
    $schedulerDateTimestamp = date("Y-m-d",strtotime(date("Y-m-d", strtotime((string) $row['last_change'])) . $row['scheduler_interval']));
...
for ($i=0; $i<count($wmsToUpdate); $i++) {
	...
	$result = json_decode((string) $resultJson);
...
SQL;
    $v = [$status, $result->message, $wmsToUpdate[$i]['wms_id']];
	$t = ['i', 's', 'i'];
	try {
